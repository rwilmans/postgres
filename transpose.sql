CREATE OR REPLACE FUNCTION transpose(	ds_in TEXT DEFAULT '',
													ds_out TEXT DEFAULT 'transpose_out',
												   	by_vars TEXT DEFAULT '',
													category_var TEXT DEFAULT '',
...
-- RAISE NOTICE 'Iteratie: (%/%), _waarde_kolom: %',i,_iteraties,_waarde_kolom;
				...
				EXECUTE FORMAT('CREATE TEMPORARY TABLE %s AS SELECT * FROM CROSSTAB(''SELECT __groepnr AS row_name, %s AS _original_category, __cat_var AS category, %s AS value FROM %s ORDER BY __groepnr'', '
							   ' ''SELECT CONCAT(%s,GENERATE_SERIES(1,(SELECT MAX(aant) FROM (SELECT COUNT(*) AS aant FROM %I GROUP BY __groepnr) AS i)))'' ) AS (__groepnr INTEGER, _original_category TEXT, %s) ;',
							   	CONCAT(_temp_table,'_', i), CONCAT('''''',_waarde_kolom,''''''), _waarde_kolom, _temp_table,
...
-- RAISE NOTICE '_categorieen: %', _categorieen;
				_query := FORMAT ('CREATE TEMPORARY TABLE %s AS SELECT * FROM CROSSTAB(''SELECT __groepnr AS row_name, %s AS _original_category, %s AS category, %s AS value FROM %s ORDER BY __groepnr'','
								   ' ''SELECT DISTINCT %s FROM %s ORDER BY 1'' ) AS (__groepnr INTEGER, _original_category TEXT, %s) ;',
								   CONCAT(_temp_table,'_', i), CONCAT('''''',_waarde_kolom,''''''), category_var, _waarde_kolom, _temp_table, category_var, _temp_table,_categorieen) ;
...
-- RAISE NOTICE 'INSERT INTO %_bijna  - (%/%) ', _temp_table, i, _iteraties;
		END LOOP;

		-- Hier de uiteindelijke tabel aanmaken
		-- En de juiste by-var kolom(men) er weer tegenaan plakken
		EXECUTE FORMAT('CREATE TEMPORARY TABLE %I AS SELECT %s, _original_category, %s FROM (SELECT DISTINCT ON (__groepnr) __groepnr, %s FROM %I) AS t, %s_bijna AS b WHERE t.__groepnr = b.__groepnr' ,
